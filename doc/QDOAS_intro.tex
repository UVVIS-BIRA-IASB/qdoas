...
\section{Overview}
The purpose of this manual is to explain how to use the different QDOAS features, and provide a basic description of their implementation.  Chapter \ref{cha:descr-algor} describes the implementation of the various algorithms in some detail, along with a brief review of their theoretical background and references to literature.  If you are already familiar with \acrfull{doas} retrievals and want to learn how to configure QDOAS, you can read the section \nameref{sec:interface} at the end of this chapter, as well as chapter \ref{cha:quickstart} -- \nameref{cha:quickstart}, which illustrates how to set up an example configuration to retrieve \NO2{} columns from ASCII spectra.  Chapter \ref{cha:properties} contains a more detailed description of the interface, explaining where the different configuration options can be found and how they can be combined.  Appendix \ref{cha:input-files-format} describes the file formats for some of the required input data.

\section{QDOAS history}
...
\subsection{General}
\begin{qdoasitem}
\item All settings can be made using the \gls{gui}.  Configuration parameters are arranged in menus with tab pages per topic.
\item A tree structure provides an overview of the different projects, with their input files and different spectral windows for analysis.
\item All the spectral windows are processed in one shot, settings for different windows may be accessed through tab pages.
\item QDOAS can process a large number of files in one run.
\item QDOAS read spectra stored in a number of different file formats (see section \ref{sec:intro-formats}) and takes into accounts the specifics of various satellite, ground-based and airborne measurements.
\item On line help in HTML format.
\end{qdoasitem}
\subsection{Plot}
\begin{qdoasitem}
\item Visualization of spectra and the results in different tab pages.
\item Configure plot contour and style.
...
\item Export of plots in different portable image formats (png, jpg).
\end{qdoasitem}
\subsection{Analysis}
\begin{qdoasitem}
\item Optical depth or intensity fitting modes.
\item shift/stretch (in nm) fully configurable for any spectral item (cross section or spectrum).
\item Automatically select a reference from the input spectra, taking into account specifics of for satellite or MAXDOAS measurements (scans).
\item Constrain the slant column density of a molecule to the value found in a previous window.
\item Filter spectra and cross sections before analysis (supported filters include Kaiser, Gaussian, boxcar, Savitsky Golay…).
\item Define gaps in the spectral window for analysis, or automatically remove spikes within fitting intervals (e.g. to eliminate bad pixels).
\item Fit an instrumental offset.
\item Configure a number of different spectral windows within a single project,
\end{qdoasitem}
...
GOME data, we recommend to use a more practical binary file format
...
\subsubsection{APEX}
APEX is a very simple netCDF format, which can be useful to analyze data from imager-type instruments which are not supported by QDOAS, or synthetic test data.  The format is described in detail in appendix \ref{sec:apex-file-format}.
...
The automatic reference selection is implemented for the native MFC binary format (without guarantee that it will work in any case) and not in the std format.  Even if it is still possible to read spectra, \textbf{no further development or support will be provided for the native format}.  BIRA-IASB has developed its own MFC binary format (MFC BIRA-IASB binary format) in which all spectra of a same day including dark currents and offsets measured the night are saved in a single file.
...
Système d'Analyse par Observation Zénithale developed by the aeronomy lab of the CNRS (JP Pommereau, F Goutail,...), France, and largely used in the \glslink{ndacc}{NDACC} network for stratospheric total ozone and \NO2 monitoring.   Both PCD/NMOS 512 and EFM 1024 (SAM) formats are supported.
...
QDOAS also provides online help in the form of HTML pages.  The
