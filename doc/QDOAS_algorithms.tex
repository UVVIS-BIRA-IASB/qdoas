...
&\cong A_\mathrm{tot}( \lambda ) N_v \frac{A_\mathrm{in}}{A_\mathrm{tot}} \left( \sigma-\sigma^{RRS} \right)
\end{align}

or

\begin{equation}
\Delta\tau\cong N_s(\lambda) \frac{A_\mathrm{in}}{A_\mathrm{tot}} \left( \sigma-\sigma^{RRS} \right) \, ,
\end{equation}
...
trace gas slant column from the previous iteration is injected
into the modified Ring cross section \eqref{eq:sigma_ring_modified} for the next iteration, until convergence is reached.  When the molecular Ring effect can be neglected, the Qdoas ring tool can be used to generate a Ring cross section $\sigma_\mathrm{R}$
...
Gaussian width. Numerous ways have been proposed to compute the Voigt
