...
least squares problem, which can typically be solved using a
Marquardt-Levenberg algorithm
(see~\citet{Marquardt1963,Bevington1969}).  Starting from given
initial values, this algorithm searches the parameter space using a
combination of Gauss-Newton and gradient descent steps.  The
Marquardt-Levenberg algorithm is an efficient minimization algorithm
for a wide range of non-linear functions, but it may converge to a
local minimum if the initial values are too far from the global
minimum.  In QDOAS, this problem can occur during the wavelength
calibration, when the initial calibration is not accurate enough (see
\nameref{sec:calibration-problems} in section
\ref{sec:analysis-problems} of the appendix).

% explain offset modelling?
In QDOAS, the non-linear least squares fit is used to find the set of parameters that
...
algorithm to the non-linear parameters: once a new set of values
$\vec{a}'$ for the non-linear parameters is chosen, the linear
...
\subsection{``Pukite'' correction}\label{pukite-terms}
The Pukite correction consists in an approximation of the wavelength
dependence of the \gls{amf}, accounting for the scattering wavelength
dependence and the dependence of the AMF on absorption strength to
first order (\citet{Pukite2010}).   For strong absorbers such as \ozone{} or \SO2{}, the wavelength dependence of the slant optical depth may be approximated as follows:

\begin{equation}
  \label{eq:pukite-o3-example}
\tau(\lambda)= S_{\lambda_0} \sigma_{\lambda}+P_1 (\lambda-\lambda_0 ) \sigma_{\lambda}+P_2 (\sigma_{\lambda}-\sigma_{\lambda_0}) \sigma_{\lambda} \,,
\end{equation}

where $\sigma_{\lambda}$ is the absorption cross section at wavelength $\lambda$, and the linearization is performed around a reference wavelength $\lambda_0$ and the corresponding cross section $\sigma_{\lambda_0}$.

The Pukite correction can be implemented with or without $I_0$-correction (see section \ref{sec:correction-solar-i_0}). The
formulation of the Pukite cross section terms differs slightly in the 2
cases.

\subsubsection{Pukite without $I_0$ correction}\label{pukite-without-i0-correction}
Without $I_0$-correction, the implementation is
straightforward. The program has to create two additional cross sections
accounting for the first-order linearization in lambda and vertical
optical depth:

\begin{align}
\mathrm{Puk}_1&=(\lambda-\lambda_0 ) \sigma_\lambda \\
\mathrm{Puk}_2&=(\sigma_\lambda-\sigma_{\lambda_0} ) \sigma_\lambda
\end{align}

The reference wavelength $\lambda_0$ is the same as the value used for the polynomial terms in the DOAS fit, which can  be configured by the user.

\subsubsection{Pukite with $I_0$-correction}\label{pukite-with-i0-correction}

When considering the $I_0$-effect, things are a bit more complicated. Let us start from the expression of the optical depth:

\begin{equation}
\tau=-\ln\left( \frac{F*\left\{ I_0\cdot \exp\left( -\mathrm{VC}\cdot\mathrm{AMF}_\lambda\cdot\sigma_\lambda\right) \right\} }{F*I_0} \right)  
\end{equation}

where $F$ is the slit function, $I_0$ is the solar spectrum, $\sigma$ is the cross section. If we consider the \gls{amf} first order expansion, we have:

\begin{equation}
\tau=-\ln\left( \frac{F*\left\{ I_0\cdot \exp\left( -\mathrm{SC}_0\cdot\sigma_\lambda-\rho\left( 1,\lambda\right) \right) \cdot\sigma_\lambda\right\} }{F*I_0} \right)
\end{equation}

where $\rho(1,\lambda)$ is a first order term and $\mathrm{SC}_0$ is not dependent on $\lambda$.

\begin{figure}
  \centering
    \includegraphics[width=0.45\textwidth]{./img/Algo_pukite_o3amf.jpg}
    \caption{Wavelength dependence of the ozone air mass factor in a
      typical \SO2 fitting window.}
    \label{fig:fig_pukite_o3amf}
\end{figure}

If we assume $\rho(1,\lambda)$ to be a small term which is generally reasonable, e.g.\ in the \SO2 fitting window (see figure \ref{fig:fig_pukite_o3amf}), we can linearize the equation (knowing that $\exp(x)=1+x$ and $\ln(1+x) \sim x$ when $x<<1$ ) and obtain:

\begin{equation}
\tau=\mathrm{SC}\cdot\sigma_{I_0\mathrm{\textnormal{-}corr}}+\frac{ F*\left\{ I_0\cdot \exp\left( -\mathrm{SC}_0\cdot\sigma_\lambda\right) \cdot\rho\left( 1,\lambda\right) \cdot\sigma_\lambda \right\} }{ F*\left\{ I_0\cdot\exp\left( -\mathrm{SC}_0\cdot\sigma_\lambda\right) \right\} }
\end{equation}

where $\sigma_{I_0\mathrm{\textnormal{-}corr}}$ is the solar $I_0$-corrected cross section and SC is the fitted slant column parameter.
Now let us consider the wavelength dependence of the AMF as in \citet{Pukite2010}:

\begin{equation}
\rho\left( 1,\lambda\right) =P_1\cdot\left( \lambda-\lambda_0 \right) +P_2\cdot\left( \sigma-\sigma_{\lambda_0}\right)
\end{equation}

and injecting in the equation above, two new terms are found (Pukite terms corrected for solar $I_0$).

\begin{align}
\mathrm{Puk1}_{I_0\mathrm{\textnormal{-}corr}}&=\frac{F*\left\{ I_0\cdot\exp\left( -\mathrm{SC}_0\cdot\sigma_\lambda\right)\cdot\lambda\cdot\sigma_\lambda\right\} }{ F*\left\{ I_0\cdot\exp\left( -\mathrm{SC}_0\cdot\sigma_\lambda\right) \right\} }-\lambda_0\cdot\sigma_{I_0\mathrm{\textnormal{-}corr}}  \\
\mathrm{Puk2}_{I_0\mathrm{\textnormal{-}corr}}&=\frac{F*\left\{ I_0\cdot\exp\left( -\mathrm{SC}_0\cdot\sigma_\lambda\right)\cdot\sigma_\lambda^2\right\} }{ F*\left\{ I_0\cdot\exp\left( -\mathrm{SC}_0\cdot\sigma_\lambda\right) \right\} }-\sigma_{\lambda_0}\cdot\sigma_{I_0\mathrm{\textnormal{-}corr}}
\end{align}

Note that the linearization around $\lambda_0$ implies the approximation :

\begin{equation}
\frac{F*\left\{ I_0\cdot\exp\left( -\mathrm{SC}_0\cdot\sigma_\lambda\right) \cdot\sigma_\lambda\right\} }{ F*\left\{ I_0\cdot\exp\left( -\mathrm{SC}_0\cdot\sigma_\lambda\right) \right\} } \cong\sigma_{I_0\mathrm{\textnormal{-}corr}}
\end{equation}
is necessary to make results independent of the choice of $\lambda_0$ and to allow for factorization of the terms in a wavelength-dependent slant column:

\begin{equation}
\tau=\mathrm{SC}\cdot\sigma_{I_0\mathrm{\textnormal{-}corr}}+P_1\cdot\mathrm{Puk1}_{I_0\mathrm{\textnormal{-}corr}}+P_2\cdot\mathrm{Puk2}_{I_0\mathrm{\textnormal{-}corr}}=\mathrm{SC}_\lambda\cdot\sigma_{I_0\mathrm{\textnormal{-}corr}}
\end{equation}

SC is the fitted slant column, corresponding to the SCD value at $\lambda_0$

and

\begin{equation}
\mathrm{SC}_\lambda\cong\mathrm{SC}+P_1\cdot\left( \lambda-\lambda_0\right) +P_2\cdot\left( \sigma_{I_0\mathrm{\textnormal{-}corr}}-\sigma_{\lambda_0}\right)
\end{equation}

\subsection{Ring Effect}\label{sec:ring-effect}
Next to elastic scattering and molecular absorption, inelastic
scattering processes in the atmosphere, in which the scattered photons
undergo a wavelength change, also affect the observed radiance.
Importantly, \acrfull{rrs} causes the so-called “filling-in” of
Fraunhofer lines due to the intensity loss at the incident wavelength
and gain at the neighbouring wavelengths to which photons are
redistributed.  This effect was first discovered
by~\citet{grainger_1962} and is referred to as the Ring effect.
Atmospheric absorption lines are also broadened by \gls{rrs} events.
This is known as the molecular Ring effect, and can be significant if
the fitting window includes absorption peaks of strong absorbers such
as \ozone{} and \SO2.  Although \gls{rrs} accounts for only a few
percent of the measured intensity, it significantly affects DOAS
measurements of scattered radiation since typical trace gas
absorptions are of the order of a percent or less.  If not properly
corrected, the Ring effect produces strongly structured residuals in
the differential optical density, due to the fact that Fraunhofer
lines do not cancel perfectly between $I$ and $I_0$. Especially in the
UV spectral range, the remaining spectral structures can by far exceed
the structures of weak atmospheric absorbers.

To account for inelastic scattering processes, the measured radiance
can be split into three terms, one term including photons having
undergone only elastic processes and two additional terms including
photons which have undergone at least one inelastic scattering
process:
\begin{equation}\label{eq:elastic_inelastic}
R_\mathrm{tot}^\lambda=R_\mathrm{el}^\lambda+R_\mathrm{inel, in}^\lambda-R_\mathrm{inel, out}^\lambda \, .
\end{equation}
Here, $R_\mathrm{el}^\lambda$ is the radiance that would be observed if only elastic scattering took place, $R^\lambda_\mathrm{inel, in}$ represents the contribution from photons originating at other wavelengths which have been scattered to wavelength $\lambda$ before reaching the detector, and $R_\mathrm{inel, out}^\lambda$ represents photons originating at wavelength $\lambda$ which have been scattered to other wavelengths.

Defining a Ring filling factor $f_\mathrm{R}$
\begin{equation}
  \label{eq:ring_filling_factor}
  f_\mathrm{R} = \frac{R_\mathrm{inel, in}^\lambda-R_\mathrm{inel, out}^\lambda }{R_\mathrm{el}^\lambda}\, ,
\end{equation}
we can rewrite \eqref{eq:elastic_inelastic} as
\begin{equation}
R_\mathrm{tot}^\lambda=R_\mathrm{el}^\lambda\cdot\left( 1+f_\mathrm{R}\right)\, .
\end{equation}

In an optical depth DOAS retrieval, we fit the log-ratio of a radiance spectrum and a reference spectrum $I_0$:
\begin{align}
\mathrm{log}\left( \frac{R_\mathrm{tot}}{I_0}\right) &=\mathrm{log}\left( \frac{R_\mathrm{el}}{I_0} \right) +\mathrm{log}\left( 1+f_\mathrm{R} \right) \\
 &\cong\mathrm{log}\left( \frac{R_\mathrm{el}}{I_0} \right) +f_\mathrm{R}\label{eq:doas_inelastic}
\end{align}
The first term of ~\eqref{eq:doas_inelastic} represents the
contribution from elastic scattering and absorption, and corresponds
to the the atmospheric species absorption cross sections and
polynomial terms in the DOAS fit.  To model the Ring effect, an
additional additional Ring ``pseudo-absorber'' cross section can be
included in the DOAS fit.  Such a cross section can be derived from
measurements ~\citep{Solomon_Schmeltekopf_Sanders_1987}, or calculated based on the typical geometry of the solar light path in a Raman scattering atmosphere (see figure \ref{fig:light_path_raman_scattering}) and the known Raman spectrum of \nitrogen{} and \oxygen{} ~\citep{Chance1997}.  Following the approach proposed by \citet{Wagner2009a}, and taking into account molecular absorption following \citet{Lerot2014}, we can express the 3 radiance terms as follows:

\begin{align}
  R_\mathrm{el}^\lambda &=T_\mathrm{el}\cdot I_0 \exp\left( -\tau\right) \label{eq:Rtot}
  \, ,\\
R_\mathrm{inel, out}^\lambda &=T_\mathrm{inel}\cdot I_0 \exp\left( -\tau\right) \label{eq:Rinel_out} \, ,\\
R_\mathrm{inel, in}^\lambda &=T_\mathrm{inel}\cdot I_0 \cdot\sigma_\mathrm{R}\cdot\ \exp\left( -\tau_\mathrm{out}-\tau_\mathrm{in}^\mathrm{RRS}\right) \label{eq:Rinel_in}\, ,
\end{align}

where $T_\mathrm{el}$ is the atmospheric transmission for photons only
undergoing elastic scattering, $T_\mathrm{inel}$ the transmission
for photons undergoing a single inelastic scattering process, $\tau$ represents the
trace gas optical depth along the elastic light path,
and $\tau_\mathrm{out}$ the optical depth along the outgoing light path
(after inelastic scattering).  $\sigma_\mathrm{R}$ is the normalised
Ring cross section as defined in \citep{Wagner2009a}:
\begin{equation}
  \label{eq:sigma_ring}
  \sigma_\mathrm{R} = \frac{\int I_0(\lambda') P(\lambda, \lambda') \mathrm{d}\lambda'}{I_0} \, ,
\end{equation}
where $P(\lambda, \lambda')$ is the probability that a photon of wavelength $\lambda'$ is scattered to the new wavelength $\lambda$.  $\tau_\mathrm{in}^\mathrm{RRS}$ is the trace
gas optical depth along the incoming light path, smoothed by Raman
scattering:

\begin{equation}
  \tau_\mathrm{in}^\mathrm{RRS} = N_\mathrm{v} A_\mathrm{in}(\lambda) \sigma^\mathrm{RRS} (\lambda)\, ,
\end{equation}

where $N_\mathrm{v}$ is the vertical column density of the absorber, $A_\mathrm{in}$ is the air mass factor along the incoming light path, and the Raman-smoothed absorber cross section $\sigma^\mathrm{RRS}$ is defined as
\begin{equation}
  \label{eq:sigma_rrs}
  \sigma^\mathrm{RRS} (\lambda) = \frac{\int I_0(\lambda') P(\lambda, \lambda') \sigma(\lambda')\mathrm{d}\lambda'}{\int I_0(\lambda') P(\lambda, \lambda') \mathrm{d}\lambda'} \,.
\end{equation}
In QDOAS, a further approximation is made and the $I_0$-terms are ignored:
\begin{equation}
  \label{eq:sigma_rrs_qdoas}
  \sigma^\mathrm{RRS} (\lambda) = \frac{\int P(\lambda, \lambda') \sigma(\lambda')\mathrm{d}\lambda' }{\int P(\lambda, \lambda') \mathrm{d}\lambda'} \,.
\end{equation}

\begin{figure}
  \centering
  \includegraphics[width=0.5\textwidth]{Algo_light_path_raman_scattering.png}
  \caption{Typical light path geometry in a Raman scattering atmosphere}
\label{fig:light_path_raman_scattering}
\end{figure}

Combining the equations \eqref{eq:ring_filling_factor} and \eqref{eq:Rtot}-\eqref{eq:Rinel_in} above, we can express the Ring filling factor as follows:
\begin{align}
f_\mathrm{R} &=\frac{T_\mathrm{inel}}{T_\mathrm{el}}\left( \sigma_\mathrm{R}\cdot\exp( \tau-\tau_\mathrm{out}-\tau_\mathrm{in}^\mathrm{RRS} ) -1 \right) \\
&=\frac{T_\mathrm{inel}}{T_\mathrm{el}}\left( \sigma_\mathrm{R}\cdot\exp( \tau_\mathrm{in}-\tau_\mathrm{in}^\mathrm{RRS} ) -1 \right)
\end{align}

The term $\frac{T_\mathrm{inel}}{T_\mathrm{el}}$  describes the probability of a photon to have undergone inelastic scattering. This term varies smoothly with wavelength, and can often be assumed constant within a typically small fitting interval. The second term describes the structured part of the Ring filling, including the impact of molecular absorption (molecular Ring). Accordingly we define a modified Ring cross section:

\begin{equation}
  \label{eq:sigma_ring_modified}
\sigma_\mathrm{R}^\prime=\sigma_\mathrm{R}\cdot\exp\left( \tau_\mathrm{in}-\tau_\mathrm{in}^\mathrm{RRS} \right) -1
\end{equation}

The term in the exponential describes the molecular absorption contained in the Ring signal. For the case of an absorber located at high altitude (e.g.\ stratospheric ozone), this term can be approximated in a simple way:

\begin{align}
  \Delta\tau =\tau_\mathrm{in}-\tau_\mathrm{in}^\mathrm{RRS} &=A_\mathrm{in}N_v\left( \sigma-\sigma^{RRS} \right) \\
&\cong A_\mathrm{tot}\left( \lambda \right) N_v \frac{A_\mathrm{in}}{A_\mathrm{tot}} \left( \sigma-\sigma^{RRS} \right)
\end{align}

or

\begin{equation}
\Delta\tau\cong N_s\left( \lambda \right) \frac{A_\mathrm{in}}{A_\mathrm{tot}} \left( \sigma-\sigma^{RRS} \right) \, ,
\end{equation}

where $\sigma$ is the absorber cross section, $\sigma^{RRS}$ the same cross section smoothed by Raman scattering, and $N_s(\lambda)$ is the slant column of the absorber, for which QDOAS will take into account the wavelength dependence if a Pukite correction is used.  $A_\mathrm{in}/A_\mathrm{tot}$  represents the mean ratio of air mass factors along the incoming and total light path.  We can further approximate this using the geometrical air mass factors.  For a satellite nadir geometry, this approximation is given by

\begin{equation}
\frac{A_\mathrm{in}}{A_\mathrm{tot}} = \frac{\mathrm{sec}\left( \mathrm{SZA}\right) }{\mathrm{sec}\left( \mathrm{SZA}\right) + \mathrm{sec}\left( \mathrm{VZA}\right) }
\end{equation}

with SZA the solar zenith angle, and VZA the line-of-sight viewing zenith angle.
In ground-based (zenith-sky or MAX-DOAS) geometry, the factor is equal to unity ($A_\mathrm{in} = A_\mathrm{tot}$).

The molecular Ring correction implies an iterative retrieval, where the fitted
trace gas slant column (and its eventual wavelength-dependence if a
Pukite correction is applied) from the previous iteration is injected
into the modified Ring cross section \eqref{eq:sigma_ring_modified} for the next iteration, until convergene is reached.  When the molecular Ring effect can be neglected, the Qdoas ring tool can be used to generate a Ring cross section $\sigma_\mathrm{R}$
following equation ~\eqref{eq:sigma_ring}.
...
\begin{equation}
  ...
  K(x,y) = \frac{y}{\pi} \int_{-\infty}^{\infty} \frac{\exp(-t^2)}{(x-t)^2+y^2}\mathrm{d}t\,,
\end{equation}
